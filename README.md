...
```python
from input_validator.validators import Validator, FunType
...
class ValidatorClass(Validator):
    ...
    def get_error(self) -> dict:
        """
          return an error dict message with the next structure:
        """
        message = {
            "title": "error title",
            "type": "error type",
...
These validators check of a list of columns has empty values.
...
class RegexValueValidator(Validator):
    ...
    def apply(self, args=None) -> bool:
        ...
        return True if re.search(regex, value) else False  # execute the comparator
